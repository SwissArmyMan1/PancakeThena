...
contract ArbitragePancakeThena is Ownable, ChainlinkClient, AutomationCompatibleInterface {
    ...
    bytes32 private jobId; // for get uint256
    string private apiUrl; //api for update gas price via chainlink
    bool private timer; //auto call arbitrage by timer
    bool private newBlock; //auto call arbitrage by new block
    bool private keeper;
    bool private value = true;
    bool private log = false;
    uint256 public gasPrice;
    uint256 private fee; //payment for chainlink in LINK
    uint256 private slippage;
    uint256 private lastUpdateTimeExchange;
    uint256 private lastUpdateTimeGas;
    uint256 private estimatedGasAmount;
    uint256 private intervalExchange; // interval for Chainlink
    uint256 private intervalGas; // interval for update gas price
    uint256 private amountWBNB; // amount bnb for exchange
    uint256 private lastBlockNumber;
    address private oracle; //address for get uint256 in BSC-mainnet (chainlink github)
    address[] private pancakePairs;
    address[] private thenaPairs;
    address private constant tokenWBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address private constant tokenLINK = 0x404460C6A5EdE2D891e8297795264fDe62ADBB75;
    address private s_forwarderAddress;
    address private trustedService;
    ...
    modifier onlyTrustedServiceOrOwner() {
        require(msg.sender == trustedService || msg.sender == owner(), "Not a trusted service or owner");
        _;
    }
    ...
    function getBalanceBNB() external view onlyTrustedServiceOrOwner returns (uint256) {
        return address(this).balance;
    }

    function getBalanceWBNB() external view onlyTrustedServiceOrOwner returns (uint256) {
        return IERC20(tokenWBNB).balanceOf(address(this));
    }

    function getBalanceERC20(address tokenContract) external view onlyTrustedServiceOrOwner returns (uint256) {
        return IERC20(tokenContract).balanceOf(address(this));
    }

    function getBalanceAllPancakeERC20() public view onlyTrustedServiceOrOwner returns (uint256[] memory) {
    ...
    function getBalanceAllThenaERC20() public view onlyTrustedServiceOrOwner returns (uint256[] memory) {
    ...
    function getBalanceLINK() public view onlyTrustedServiceOrOwner returns (uint256) {
        return IERC20(tokenLINK).balanceOf(address(this));
    }

    function getPancakePairs() external view onlyTrustedServiceOrOwner returns (address[] memory) {
        return pancakePairs;
    }

    function getThenaPairs() external view onlyTrustedServiceOrOwner returns (address[] memory) {
        return thenaPairs;
    }

    function getTimerAndNewBlockEvents() external view onlyTrustedServiceOrOwner returns (bool, bool) {
        return (timer, newBlock);
    }

    function getGasPrice() external view onlyTrustedServiceOrOwner returns(uint256) {
        return(gasPrice);
    }

    function getGasRequestSettings() external view onlyTrustedServiceOrOwner returns(string memory, string memory, address, uint256) {
        return(apiUrl, string(abi.encodePacked(jobId)), oracle, fee);
    }

    function getLog() external view onlyTrustedServiceOrOwner returns(bool){
        return(log);
    }

    function setGasRequestSettings(string memory _apiUrl, string memory _jobId, address _oracle, uint256 _fee) external onlyTrustedServiceOrOwner {
        apiUrl = _apiUrl;
    ...
    function setIntervalExchange(uint256 _intervalExchange) external onlyTrustedServiceOrOwner {
        intervalExchange = _intervalExchange;
    }

    function setIntervalGas(uint256 _intervalGas) external onlyTrustedServiceOrOwner {
        intervalGas = _intervalGas;
    }

    function setTypeOfEvent(bool setTimer, bool setBlock) external onlyTrustedServiceOrOwner {
        require(!(setTimer && setBlock), "setTimer and setBlock cannot both be true");
        timer = setTimer;
        newBlock = setBlock;
    }

    function setForwarderAddress(address forwarderAddress) external onlyTrustedServiceOrOwner { //setup of address of chainlink keepers
        s_forwarderAddress = forwarderAddress;
    }

    function setEstimatedGasAmount(uint256 _estimatedGasAmount) external onlyTrustedServiceOrOwner {
        estimatedGasAmount = _estimatedGasAmount;
    }

    function setGasPriceManual(uint256 _gasPrice) external onlyTrustedServiceOrOwner {
        gasPrice = _gasPrice * estimatedGasAmount;   //set in wei
    }

    function setSlippage(uint256 _slippage) external onlyTrustedServiceOrOwner {
    ...
    function setKeeper(bool _keeper) external onlyTrustedServiceOrOwner {
        keeper = _keeper;
    }

    function setLog(bool _log) external onlyTrustedServiceOrOwner {
        log = _log;
    }

    function addPairs(address _pancakePair, address _thenaPair) external onlyTrustedServiceOrOwner {
        pancakePairs.push(_pancakePair);
        thenaPairs.push(_thenaPair);
    }

    function removePairs(uint256 index) external onlyTrustedServiceOrOwner {
    ...
    function checkUpByService() external onlyTrustedServiceOrOwner {
        require(keeper != true);
        if (timer) {
            _executeArbitrage();
        }
    ...
    function requestGasPriceData() external onlyTrustedServiceOrOwner { //external request for update gas price
        _requestGasPriceData();
    }
    ...
    function checkArbitrageOpportunity() internal view returns  (uint256 profitIndex, 
        ...
        return  (profitIndex, direction, reserveSlipFeePancakeBNB, reserveSlipFeePancakeOther, 
    ...
    function executeArbitrage() external onlyTrustedServiceOrOwner {
        _executeArbitrage();
    }
